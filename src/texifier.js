...
//             [" ", "\\;"],
    ...
    texify(text) {
        ...
        let mg = res.match(/if\s+(.+?)\s+then\s+(.+?)\s+else/g);
        let findex = 0;
        if (mg) {
            for (let i = 0; i<mg.length; i++) {
                let m = mg[i].match(/if\s+(.+?)\s+then\s+(.+?)\s+else/);
                if(m && !m[1].includes("if") && !m[1].includes("then") && !m[1].includes("else") && !m[2].includes("if") && !m[2].includes("then") && !m[2].includes("else")) {
                    let beginning_index = res.indexOf("if", findex);
                    let beginning_of_end = res.indexOf("else", findex);
